...
public class Resource {
	...
	@GET
	@Path("/{IBAN}&{pincode}")
	public CheckPincodeResponse getUserID(@PathParam("IBAN") String iban, @PathParam("pincode") long pincode) {
		...
		return response; // ("{\"userid\":\"" + serv.getUserID(iban, pincode) +
							// "\"}"); --> This is the old JSON object, but we
							// should use API!
	}

	/*
	 * This and the method above could have been done together, but because the
	 * DAO methods should have only 1 specific task, I spread the calls into 2
	 * different methods.
	 */
	@GET
	...
	public WithdrawResponse withdraw(@PathParam("IBAN") String iban, WithdrawRequest request) {
		...
		transactieNummer = serv.withdraw(iban, request.getAmount());

		if (transactieNummer > 0) {
			response.setResponse("Dank u voor pinnen.");
			response.setTransactionNumber(transactieNummer);
			return response;
		} else {
